package _04_映射Map._03_LinkedHashMap;
...
public class LinkedHashMap<K,V> extends HashMap<K,V> {
    ...
    public void traversal(Visitor<K, V> visitor) {
        ...
        LinkedNode<K, V> node = first;
        while (node != null) {
            if (visitor.visit(node.getKey(), node.getValue())) return;
            node = node.next;
        }
    }

    @Override
    protected void LinkedRemove(Node<K, V> targetNode, Node<K, V> removedNode) {
        LinkedNode<K, V> node1 = (LinkedNode<K, V>) targetNode;
